 ...
 *
 */
package com.android.hp.ros.rosbridge.implementation;

import com.android.hp.ros.message.Message;
import com.android.hp.ros.rosbridge.indication.Indication;
import com.android.hp.ros.rosbridge.operation.Wrapper;
...
public class JSON {
    ...
    private static Message convertJSONObjectToMessage(JSONObject jo, Class c, Registry<Class> r) {
        ...
        try {
            ...
            for (Field f : c.getFields()) {
                ...
                if (lookup != null) {
                    Object value = convertElementToField(f.getName(), lookup, fc, f, r);
                    f.set(result, value);
                }
            }
            return result;
    ...
    //    recursively via convertElementToField.
    private static Object convertJSONArrayToArray(String name, JSONArray ja, Class c, Registry<Class> r) {
        ...
        for (int i = 0; i < ja.size(); i++) {
            ...
            if (lookup != null) {
                if (lookup.getClass().equals(JSONObject.class))
                    ...
                    value = convertJSONArrayToArray(name, (JSONArray) lookup, c.getComponentType(), r);
                else
                    value = convertJSONPrimitiveToPrimitive(name, lookup, c);
    ...
    //     NOTE: This relies on later versions of the JDK providing 
    //           the fields in order.
    private static Message convertJSONArrayToMessage(String name, JSONArray ja, Class c, Registry<Class> r) {
        try {
            Message result = (Message) c.newInstance();
            int arrayIndex = 0;
            for (Field f : c.getFields()) {
                ...
                if (lookup != null) {
                    Object value = convertElementToField(name, lookup, fc, f, r);
                    f.set(result, value);
                }
            }
    ...
    private static Object convertElementToField(String name, Object element, Class fc, Field f, Registry<Class> r) {
        ...
        if (element.getClass().equals(JSONObject.class)) {
            ...
            if (Indication.asArray(f))
                value = convertJSONArrayToMessage(name, (JSONArray) element, fc, r);
            else value = convertJSONArrayToArray(name, (JSONArray) element, fc, r);
        }
        else {
            //System.out.println("JSON.convertElementToField: Primitive " + element);
            if (Indication.isBase64Encoded(f))
                value = convertBase64JSONStringToByteArray(element);
            else value = convertJSONPrimitiveToPrimitive(name, element, fc);
        }
    ...
    public static Object convertJSONPrimitiveToPrimitive(String name, Object o, Class c) {
        Object result = o;
        ...
        } else if (name != null && name.equals("data")) {
            result = o.toString();
        }
        return result;
    }
